...
void WindowControls::Init(SDL_Window* window_main)
{
    window.Init("Controls", win_width, win_height, win_width, win_height, SDL_WINDOW_HIDDEN);
    window.OnOpen = std::bind(&WindowControls::Open,this);
    window.OnClose = std::bind(&WindowControls::Close,this);
    window.OnUpdate = std::bind(&WindowControls::Update, this);
    ...
    for (int i = 0; i < Joypad::num_keys; i++)
    {
        ...
        button_keymap[0][i] = window.AddButton(95, 40 + i * 30, 100, 21, std::string(SDL_GetKeyName(keymaps[0][i])), std::bind(&WindowControls::OnButtonClick, this, 0, i));
        offset = 10 + i * 30;
    }
    ...
    for (int i = 0; i < Joypad::num_keys; i++)
    {
        ...
        button_keymap[1][i] = window.AddButton(95, offset + 130 + i * 30, 100, 21, std::string(SDL_GetKeyName(keymaps[1][i])), std::bind(&WindowControls::OnButtonClick, this, 1, i));
    }
    
}

void WindowControls::OnButtonClick(int joynum, int butnum)
{
...
void WindowControls::Refresh(bool cancel)
{
    for (int j = 0; j < 2; j++)
    {
        for (int i = 0; i < Joypad::num_keys; i++)
